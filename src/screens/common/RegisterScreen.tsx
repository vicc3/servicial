...
const RegistrationScreen: React.FC<Props> = ({ navigation, route }) => {
  const { role } = route.params; // <-- Obtenemos el rol de los parámetros
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await firestore().collection('users').doc(user.uid).set({
        ...
        role: role // <-- El rol se define aquí
      });

      Alert.alert('Éxito', `¡Registro como ${role} completado correctamente!`);
    ...
    <View style={styles.fullScreenContainer}> 
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        <Title style={styles.mainTitle}>
          Registro de {role === 'user' ? 'User' : 'Worker'}
        </Title>
